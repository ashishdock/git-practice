// A simple program that lets you experiment with the effects of changing the thresholdand parallelism of a ForkJoinTask.
...
class Transform extends RecursiveAction
{
	...
	// This isthe method in which parallel computation will occur.
	protected void compute()
	{
		// If number of elements is below the sequential threshold, then process sequentially.
		if((end - start) < seqThreshold)
		{
			// The following code assigns an element at an even index the square root of its original value. An element at an odd index is assigned its cube root. This code is designed to simply consume CPU time so that the effects of concurrent execution are more readily observable.
